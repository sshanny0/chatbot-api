...
def get_qna_data(page: int = 1, limit: int = 10):
    ...
    data = pd.read_sql(
        "SELECT a.ID AS id, a.question AS question, a.keyword AS category, a.answer AS answer, b.hyperlink AS hyperlink, b.tag AS tag, a.status AS status "
    ...
    total = pd.read_sql("SELECT COUNT(*) as count FROM hesk_chatbot_qna", con=mydb)
...
def update_qna_data(
    ...
    sql_link = "UPDATE hesk_chatbot_link SET hyperlink = %s, tag = %s WHERE qna = %s"
